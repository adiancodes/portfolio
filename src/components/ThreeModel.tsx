...
export const ThreeModel = ({
  ...
  const blurShadow = useCallback((amount: number) => {
    if(!blurPlane.current || !horizontalBlurMaterial.current || !renderTarget.current || !renderer.current || !renderTargetBlur.current || !verticalBlurMaterial.current || !shadowCamera.current) return;
    blurPlane.current.visible = true;

    // Blur horizontally and draw in the renderTargetBlur
    blurPlane.current.material = horizontalBlurMaterial.current;
    (blurPlane.current.material as any).uniforms.tDiffuse.value = renderTarget.current.texture;
    ...
    (blurPlane.current.material as any).uniforms.tDiffuse.value = renderTargetBlur.current.texture;
  ...
  const renderFrame = useCallback(() => {
    if(!scene.current || !depthMaterial.current || !renderer.current || !renderTarget.current || !shadowCamera.current || !modelGroup.current || !camera.current) return;
    const blurAmount = 5;
  ...
  return (
    <div
      ...
      {...rest}
    >
      <canvas className="w-full h-full block" ref={canvas} />
...
const Device = ({
  ...
  const [loadDevice, setLoadDevice] = useState<any>();
  const reduceMotion = useReducedMotion();
  const placeholderScreen = createRef<any>();
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [placeholder, gltf] = await Promise.all([
        texture?.placeholder ? await textureLoader.loadAsync(texture.placeholder) : null,
        await modelLoader.loadAsync(url),
      ]);

      if(modelGroup.current) modelGroup.current.add(gltf.scene);

      gltf.scene.traverse(async (node: any) => {
        if (node.material) {
          node.material.color = new Color(0x1f2025);
        }

        if (node.name === MeshType.Screen && placeholder) {
          ...
          loadFullResTexture = async () => {
            ...
            animate(1, 0, {
              onUpdate: value => {
                if(placeholderScreen.current) placeholderScreen.current.material.opacity = value;
                renderFrame();
              },
            });
          };
        } else if (node.name === MeshType.Screen && texture?.url) {
            // simpler version without placeholder
            const fullSize = await textureLoader.loadAsync(texture.url);
            await applyScreenTexture(fullSize, node);
        }
      });
      ...
      if (model.animation === ModelAnimationType.LaptopOpen) {
        playAnimation = () => {
          const frameNode = gltf.scene.children.find(
            (node: any) => node.name === MeshType.Frame
          );
          if(!frameNode) return;
  ...
  useEffect(() => {
    ...
    const onModelLoad = async () => {
      ...
      if(loadFullResTexture) await loadFullResTexture();
  ...
  return null; // The device is rendering inside the canvas of the parent
};
